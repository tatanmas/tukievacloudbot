...
- **Fallback:** Siempre **Claude primary**, **Kimi fallback**. Si Claude falla (límite, 429, etc.), el gateway debe intentar Kimi sin cortar. En clawdbot hay un bug: el error "API usage limits" a veces no dispara el fallback; si pasa, reportar/actualizar clawdbot. No cambiar el orden (Claude = primary).
...
**Dónde se configura:** `.clawdbot/clawdbot.json` → `agents.defaults.model.primary` y `agents.defaults.model.fallbacks`. No poner API keys en ese archivo; van en `.env`. Detalle de que el fallback es **hexagonal** (gateway, no LLM): **docs/FALLBACK-MODELO.md**.
